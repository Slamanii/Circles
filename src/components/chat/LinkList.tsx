...
export function extractLinks(messages: { id: string; content: string; senderName: string; type: string }[]): LinkItem[] {
    ...
    for (const msg of messages) {
        ...
        if (found) {
            found.forEach(url => links.push({ id: `${msg.id}-${url}`, url, sentBy: msg.senderName }));
        }
    }
    return links;
}
...
export function LinkList({ links }: Props) {
    ...
    if (!links.length) {
        return <Text style={styles.empty}>No links shared yet</Text>;
    }
            ...
            renderItem={({ item }) => (
                ...
                >
                    ...
                    <View style={styles.info}>
                        ...
                        <Text style={styles.sentBy}>Shared by {item.sentBy}</Text>
                    </View>
                </TouchableOpacity>
            )}
        />
...
const styles = StyleSheet.create({
    ...
    sentBy: { color: "#64748B", fontSize: 12, marginTop: 2 },
});
